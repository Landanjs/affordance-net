...
if __name__ == '__main__':
    ...
    for _ in range(len(list_boxes)*100):
        ...
        print('Pixel coordinates:', coords[:, 315*403])
        ...
        coords_3D = coords
        print('3D coordinates:', coords_3D[:, 315*403]/1000)
        # make homogeneous coordinates
        coords_hom = np.concatenate([coords_3D/1000, np.ones((1, width*height))], axis=0)
        print('Rviz coordinates:', coords_hom[:, 315*403])
        ...
        if dval != 'nan':
            ...
            obj_pose_3D.pose.position.y = round(p3Dc_marker[1], 2) + 0.3
